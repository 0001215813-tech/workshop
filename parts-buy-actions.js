...
async function createPurchase(id,p,q){
 ...
 const order={partId:id,partName:partName(p,id),quantity:q,unit:value(p,'unit','unidade')||'un',status:'pending',createdAt:firebase.database.ServerValue.TIMESTAMP,etaAt:eta,deliverySeconds:180};
...
function buyUI(card,id,p){if(card.querySelector('.parts-buy-btn')){renderDelivery(card,id);return}const b=document.createElement('button');b.type='button';b.className='parts-buy-btn';b.innerHTML='<i class="fa-solid fa-cart-shopping" style="margin-right:7px"></i>Comprar Peça';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();let box=card.querySelector('.parts-buy-box');if(box){box.classList.toggle('open');return}box=document.createElement('div');box.className='parts-buy-box open';box.innerHTML='<div style="font-size:11px;color:#94a3b8;margin-bottom:6px">Quantidade a comprar</div><input class="pbb-input" type="number" min="1" step="1" value="1"><div style="font-size:11px;color:#94a3b8;margin-top:8px">Tempo de entrega: <strong style="color:#e2e8f0">3 minutos</strong></div><div class="pbb-actions"><button type="button" class="pbb-cancel">Cancelar</button><button type="button" class="pbb-confirm">Confirmar compra</button></div>';card.appendChild(box);box.querySelector('.pbb-cancel').onclick=()=>box.remove();box.querySelector('.pbb-confirm').onclick=async()=>{const q=Number(box.querySelector('.pbb-input').value);if(!Number.isInteger(q)||q<=0){alert('Informe uma quantidade inteira maior que zero.');return}const btn=box.querySelector('.pbb-confirm');btn.disabled=true;btn.textContent='Registrando...';try{await createPurchase(id,p,q);box.remove();pendingOrders=await loadOrders();renderDelivery(card,id);alert('Compra confirmada! A mercadoria ficará em trânsito por 3 minutos e só depois entrará no estoque.')}catch(err){console.error(err);alert('Não foi possível registrar a compra no Firebase.');btn.disabled=false;btn.textContent='Confirmar compra'}}});card.appendChild(b);renderDelivery(card,id)}
...
window.buyInventoryPart=async function(id,p){const q=prompt('Quantas unidades de "'+partName(p,id)+'" deseja comprar?','1');if(q===null)return;const n=Number(q);if(!Number.isInteger(n)||n<=0)return alert('Informe uma quantidade inteira maior que zero.');try{await createPurchase(id,p,n);pendingOrders=await loadOrders();alert('Compra confirmada! A mercadoria ficará em trânsito por 3 minutos e só depois entrará no estoque.')}catch(e){console.error(e);alert('Não foi possível registrar a compra no Firebase.')}};
