...
(function(){
  ...
  function ensureAlert(){
    ...
    document.getElementById('sourceFeatureAlertOS').onclick=()=>{ if(window.newOS) window.newOS(); };
    return el;
  }

  window.simulateCase=function(){
    try{
      const eqs=window.state?.equipments || {};
      const id=Object.keys(eqs)[0];
      if(window.root && id){
        window.root.child('equipments/'+id).update({status:'Em Manutenção',horimetro:505});
        const e=eqs[id];
        ...
        alert('Cadastre pelo menos um equipamento antes de simular o caso.');
      }
  ...
  function installEquipmentActions(){
    ...
    if(!list || list.dataset.sourceActions) return;
    list.dataset.sourceActions='1';
    const observer=new MutationObserver(()=>{
      list.querySelectorAll('.card').forEach(card=>{
        if(card.querySelector('.source-eq-actions')) return;
        const title=card.querySelector('h3');
        if(!title) return;
        const wrap=document.createElement('div');
        wrap.className='source-eq-actions grid grid-cols-2 gap-2 mt-4';
        const btnOS=document.createElement('button');
        btnOS.className='bg-blue-600/15 hover:bg-blue-600 text-blue-300 hover:text-white border border-blue-500/30 rounded-xl px-3 py-2 text-xs font-black transition';
        btnOS.innerHTML='<i class="fa-solid fa-file-circle-plus mr-1"></i> Abrir O.S.';
        btnOS.onclick=()=>window.newOS&&window.newOS();
        const btnQR=document.createElement('button');
        btnQR.className='bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl px-3 py-2 text-xs font-black transition';
        btnQR.innerHTML='<i class="fa-solid fa-qrcode mr-1"></i> QR do ativo';
        btnQR.onclick=()=>showAssetQR(title.textContent);
  ...
  function showAssetQR(name){
    ...
    if(!modal){
      ...
      modal.innerHTML='<div class="card w-full max-w-md p-6 text-center"><div class="flex justify-between items-center"><div class="text-lg font-black">QR Code do Ativo</div><button id="sourceQRClose" class="w-9 h-9 rounded-lg bg-slate-800">✕</button></div><div id="sourceQRBox" class="my-5 flex justify-center min-h-[220px] items-center"></div><div id="sourceQRName" class="font-bold"></div><p class="text-xs text-slate-500 mt-2">Use este código para identificar o equipamento na oficina.</p><div class="flex gap-2 mt-5"><button id="sourceQRPrint" class="flex-1 bg-blue-600 hover:bg-blue-500 rounded-xl py-2 font-black text-sm">🖨️ Imprimir</button><button id="sourceQROpen" class="flex-1 bg-slate-800 hover:bg-slate-700 rounded-xl py-2 font-black text-sm">Emitir O.S.</button></div></div>';
      document.body.appendChild(modal);
      modal.querySelector('#sourceQRClose').onclick=()=>modal.classList.add('hidden');
      modal.querySelector('#sourceQROpen').onclick=()=>{modal.classList.add('hidden');window.newOS&&window.newOS()};
      modal.querySelector('#sourceQRPrint').onclick=()=>window.print();
    }
    modal.querySelector('#sourceQRName').textContent=name;
    const box=modal.querySelector('#sourceQRBox'); box.innerHTML='';
    if(window.QRCode){
      new QRCode(box,{text:'SENAI-CMMS|ATIVO|'+name,width:210,height:210,correctLevel:QRCode.CorrectLevel.M});
    }else{
      const canvas=document.createElement('div');canvas.className='rounded-xl bg-slate-900 border border-slate-700 p-8 text-slate-400 text-sm';canvas.innerHTML='QR Code indisponível no momento.<br>O restante do aplicativo continua funcionando.';box.appendChild(canvas);
    }
  ...
  function installPrintActions(){
    const osList=document.getElementById('osList'); if(!osList || osList.dataset.printActions) return;
    osList.dataset.printActions='1';
    const observer=new MutationObserver(()=>{
      osList.querySelectorAll('tr').forEach(row=>{
        if(row.querySelector('.source-print-os')) return;
        const first=row.querySelector('td'); if(!first) return;
        const id=(first.textContent||'').replace('#','').trim();
        const action=row.lastElementChild; if(!action) return;
        const b=document.createElement('button'); b.className='source-print-os ml-1 bg-slate-700 hover:bg-slate-600 rounded-lg px-2 py-1.5 text-xs'; b.title='Imprimir O.S.'; b.innerHTML='🖨️';
        b.onclick=()=>printOSRow(row,id);
  ...
  function installAdvancedDashboard(){
    ...
    let c1,c2;
    const render=()=>{
      if(!window.Chart || !window.state)return;
      const orders=Object.values(window.state.orders||{}), eqs=Object.values(window.state.equipments||{});
      const types=['Preventiva','Corretiva','Preditiva']; const tdata=types.map(t=>orders.filter(o=>o.type===t).length);
      const sdata=['Operando','Em Manutenção','Parado'].map(s=>eqs.filter(e=>(e.status||'Operando')===s).length);
      if(c1)c1.destroy(); if(c2)c2.destroy();
      c1=new Chart(document.getElementById('sourceTypeChart'),{type:'doughnut',data:{labels:types,datasets:[{data:tdata}]},options:{responsive:true,plugins:{legend:{labels:{color:'#cbd5e1'}}}}});
      c2=new Chart(document.getElementById('sourceAssetChart'),{type:'doughnut',data:{labels:['Operando','Em Manutenção','Parado'],datasets:[{data:sdata}]},options:{responsive:true,plugins:{legend:{labels:{color:'#cbd5e1'}}}}});
    };
    const oldRender=window.render;
    if(typeof oldRender==='function' && !oldRender.__sourceWrapped){
      const wrapped=function(){oldRender.apply(this,arguments);setTimeout(render,0)}; wrapped.__sourceWrapped=true; window.render=wrapped;
    }
    setTimeout(render,500);
  }

  function installSimulationButton(){
    const btn=[...document.querySelectorAll('button')].find(b=>(b.textContent||'').includes('SIMULAR CASO'));
    if(btn) btn.onclick=window.simulateCase;
  }

  function init(){
    installSimulationButton(); installEquipmentActions(); installPrintActions(); installAdvancedDashboard();
    setTimeout(()=>{installSimulationButton();installEquipmentActions();installPrintActions();},1000);
  }
