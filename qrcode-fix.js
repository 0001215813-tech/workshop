/* QR Code robusto: substitui o modal antigo e não depende de QRCodeJS. */
(function(){
  'use strict';
  function buildQRUrl(name){
    const pdfUrl=new URL('./?asset='+encodeURIComponent(name||'Ativo')+'&pdf=1&v=pdf4',window.location.href).href;
  ...
  function direct(name){
    ...
    img.width=210;img.height=210;img.alt='QR Code do ativo '+(name||'Ativo');
    img.style.cssText='width:210px;height:210px;background:#fff;padding:8px;border-radius:12px;display:block';
  ...
  document.addEventListener('click',function(e){
    ...
    if(text.includes('qr do ativo')){
      e.preventDefault();e.stopImmediatePropagation();
      const card=btn.closest('.card');
      const title=card&&card.querySelector('h3');
      direct(title?title.textContent.trim():'Ativo');
    }
  },true);
})();
